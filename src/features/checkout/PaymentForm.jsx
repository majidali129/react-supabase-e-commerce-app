...
const PaymentForm = () => {
    ...
    function onSubmit ({paymentMethod , cardNumber, cardHolderName, expireDate, cvvNumber}) {
  ...
  return (
   <section className="md:w-[50%] md:mx-auto bg-stone-50 mt-[1rem] py-10 px-3 rounded-md">
    ...
    <FormRow className='flex-col'  error={errors?.paymentMethod?.message}>
        ...
        <select  disabled={gettingPaymentMethod}
        name='paymentMethod'
        {...register('paymentMethod')}
        className="w-full py-2 bg-inherit shadow-lg outline-none border-none rounded-sm  ring-2 ring-stone-300" 
        >
    ...
    <FormRow className='flex-col'  error={errors?.cardNumber?.message}>
        ...
        <FormInput disabled={gettingPaymentMethod} id='cardNumber' placeholder='2390-4567-9034-1243' name='cardNumber' {...register('cardNumber', {
    ...
    <FormRow  className='flex-col'  error={errors?.cardHolderName?.message}>
        ...
        <FormInput disabled={gettingPaymentMethod} id='cardHolderName' placeholder='Majid Ali' name='cardHolderName' {...register('cardHolderName', {
    ...
    <FormRow  className='flex-col' error={errors?.expireDate?.message}>
        ...
        <FormInput disabled={gettingPaymentMethod} id='expireDate' placeholder='MM/YY' name='expireDate' {...register('expireDate', {
    ...
    <FormRow  className='flex-col' error={errors?.cvvNumber?.message}>
        ...
        <FormInput disabled={gettingPaymentMethod} id='cvvNumber' name='cvvNumber' {...register('cvvNumber', {
            required: 'Field can\'t be empty'
        })}  />
    </FormRow>

    <FormRow >
    <Button  disabled={gettingPaymentMethod}
    type="submit" 
    variation='primary' 
    className='w-full'
    > {gettingPaymentMethod? 'Making': 'MAKE A PAYMENT'}</Button>
    </FormRow>
